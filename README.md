...
The project is structured as follows:
 ...
 - [metrics_pnet_2gru.ipynb](https://github.com/YunfeiGu/APSNet/blob/main/metrics_pnet_2gru.ipynb) is a jupyter notebook script that used for testing APSNet_GG (explained in Section IV Exp#2 in our paper) model on metrics of TPR, FPR, AUC, F1-score.
...
- To facilitate the software dependencies setup, [/APSNet/environment.yml](https://github.com/YunfeiGu/APSNet/blob/main/environment.yml) which illustrates all the libraries and dependencies versions is provided. It can be loaded as the initial setup in conda environment.
## SMART Dataset
...
The procedure for SMART data preprocessing is explained in [smart-preprocess/README.md](https://github.com/YunfeiGu/APSNet/blob/main/smart-preprocess/README.md).
...
- [smart-preprocess/npy/partial_statistics_mc1_model1.npy](https://github.com/YunfeiGu/APSNet/blob/main/smart-preprocess/npy/partial_statistics_mc1_model1.npy)
- [smart-preprocess/npy/partial_statistics_mc1_model2.npy](https://github.com/YunfeiGu/APSNet/blob/main/smart-preprocess/npy/partial_statistics_mc1_model2.npy)
- [smart-preprocess/npy/bad_mc1_model1.npy](https://github.com/YunfeiGu/APSNet/blob/main/smart-preprocess/npy/bad_mc1_model1.npy)
- [smart-preprocess/npy/bad_mc1_model2.npy](https://github.com/YunfeiGu/APSNet/blob/main/smart-preprocess/npy/bad_mc1_model2.npy)
...
- Due to the Intellectual Property policy of the Raw SSD BEC aging data, we only provide the normalized and well-processed SSD BEC aging data in [APSNet/data/aging.npy](https://github.com/YunfeiGu/APSNet/blob/main/data/aging.npy).
...
In [/APSNet/data](https://github.com/YunfeiGu/APSNet/blob/main/data), there are one npy file named aging.npy and 8 subfolders. [aging.npy](https://github.com/YunfeiGu/APSNet/blob/main/data/aging.npy) is the SSD Aging BEC Dataset we proposed and collected. It is the dataset that needs to be fed in the sub-network 2.1b of APSNet. In each subfolder, there is an SSD SMART dataset that needs to be fed in the sub-network 2.1a of APSNet and other baseline models including RF, SVM, LR, DT, KNN, NN, and LSTM. (To understand the APSNet network structure and how these two types of dataset feed-in APSNet, please check Figure 8, Section III-B and III-C in our paper). 

These SMART datasets in each subfolder have been processed by [APSNet/slidingWindow.py](https://github.com/YunfeiGu/APSNet/blob/main/slidingWindow.py) already, in order to train and test all learning models for evaluating N days lookahead failure prediction performance.
...
- [APSNet/data/aging.npy](https://github.com/YunfeiGu/APSNet/blob/main/data/aging.npy): Processed SSD Aging BEC Dataset. It is used by the sub-network 2.1b of APSNet.
- [APSNet/data/5_days_lookahead/smart_train.npy](https://github.com/YunfeiGu/APSNet/blob/main/data/5_days_lookahead/smart_train.npy): Processed SMART dataset used for all learning model training on SSD failure prediction in 5 days ahead. 
- [APSNet/data/5_days_lookahead/test_labels.npy](https://github.com/YunfeiGu/APSNet/blob/main/data/5_days_lookahead/test_labels.npy): Labels of aforementioned SMART traininig dataset.
- [APSNet/data/5_days_lookahead/smart_test.npy](https://github.com/YunfeiGu/APSNet/blob/main/data/smart_test.npy): Processed SMART dataset used for all learning model testing on SSD failure prediction in 5 days ahead. 
- [APSNet/data/5_days_lookahead/test_labels.npy](https://github.com/YunfeiGu/APSNet/blob/main/data/test_labels.npy): Labels of aforementioned SMART testing dataset.
...
For example, if you want to train the RF learning model for evaluating the performance on 5 days lookahead SSD failure prediction, input 5 and the well-trained RF learning model will be saved as [rf.pkl](https://github.com/YunfeiGu/APSNet/blob/main/trained_model/5_days_lookahead/rf.pkl) in subfolder [APSNet/trained_model/5_days_lookahead](https://github.com/YunfeiGu/APSNet/blob/main/trained_model/5_days_lookahead).
...
To reproduce all experiment results, we provide the jupyter notebook scripts to evaluate the failure prediction performance on all metrics in various days lookahead. Please read the check-list artifact check-list in Section~\ref{sec:check-list} and model testing instruction in the [APSNet/README.md](https://github.com/YunfeiGu/APSNet/blob/main/README.md) file. 
...
- [APSNet/metrics_lstm.ipynb](https://github.com/YunfeiGu/APSNet/blob/main/metrics_lstm.ipynb)
- [APSNet/metrics_nn.ipynb](https://github.com/YunfeiGu/APSNet/blob/main/metrics_nn.ipynb)
- [APSNet/metrics_rf_knn_lr_dt_svm.ipynb](https://github.com/YunfeiGu/APSNet/blob/main/metrics_rf_knn_lr_dt_svm.ipynb)
- [APSNet/metrics_apsnet.ipynb] (https://github.com/YunfeiGu/APSNet/blob/main/metrics_apsnet.ipynb)
...
Figure 12.a are statistics that obtained from ROC curves printed by [APSNet/metricsapsnet.ipynb](https://github.com/YunfeiGu/APSNet/blob/main/metrics_apsnet.ipynb) via setting N days lookahead with 5, 7, 14, 30, 45, 60, 90 and 120.

Figure 12.b Figure 12.c and Figure 12.d compare statistics that obtained from ROC curves printed by [APSNet/metrics_lstm.ipynb](https://github.com/YunfeiGu/APSNet/blob/main/metrics_lstm.ipynb) and [APSNet/metrics_nn.ipynb](https://github.com/YunfeiGu/APSNet/blob/main/metrics_nn.ipynb) with which are got from [APSNet/metrics_apsnet.ipynb](https://github.com/YunfeiGu/APSNet/blob/main/metrics_apsnet.ipynb) by setting N with 7, 30 and 60. 
...
For Exp#2:
- [APSNet/metrics_pnet_2gru.ipynb](https://github.com/YunfeiGu/APSNet/blob/main/metrics_pnet_2gru.ipynb)
- [APSNet/metrics\_pnet\_2lstm.ipynb](https://github.com/YunfeiGu/APSNet/blob/main/metrics_2lstm.ipynb)
- [APSNet/metrics_gru_lstm.ipynb](https://github.com/YunfeiGu/APSNet/blob/main/metrics_gru_lstm.ipynb)
...
For Exp#3:
- [APSNet/metrics_pnet.ipynb](https://github.com/YunfeiGu/APSNet/blob/main/metrics_pnet.ipynb.ipynb)
...
- After login the container, the main code is shown in directory: /root/apsnet
 ...
 - The the file structure in docker image is the same as using local environment as described in [/APSNet/README.md](https://github.com/YunfeiGu/APSNet/blob/main/README.md), except environment.yml, because all soft dependencies have been included in the container already.
...
All the artifact instructions using container are the same as using the local environment that is described in described in [/APSNet/README.md](https://github.com/YunfeiGu/APSNet/blob/main/README.md).
